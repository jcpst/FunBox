...
| **Knob 4**     | **SENSE** — Envelope sensitivity (1×–20×, logarithmic). Set to minimum to enable expression pedal envelope control. |
...
All main parameters can be controlled in real time via MIDI Continuous Controllers. MIDI CC has the highest priority — it overrides both preset and physical knob values. Moving the corresponding physical knob reclaims control from MIDI.
...
```sh
cd software/DwarfPlanets/Ceres
make
